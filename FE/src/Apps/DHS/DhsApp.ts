...
import { SetLayoutOperation, UpdateProgressInfo } from './model/DhsPanelOps';
import MessageTips from './components/MessageTips.vue';
import { DshCommandSendHelper } from '../DHS/model/DhsCommandSendhelper';

@Options({
  props: {},
  components: {
    'message-tips': MessageTips
  }
})
export default class DrvApp extends Vue implements BaseApp {
  ...
  public registerType(): void {
    ...
    this.modelContainer.register(UpdateProgressInfo);
  }
    ...
    if (statusHandleModel) {
      statusHandleModel.beCreated.then(result => {
        if (result) {
          ...
          /* DshCommandSendHelper.sendInitSeriesList(
            this.modelContainer,
            seriesUIDArray as any[]
          ); */
        }
      });
    } else {
